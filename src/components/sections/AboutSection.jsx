...
export const aboutData = [
  {
    ...
    info: [
      ...
      },
      {
        ...
        icons: [
          { Icon: TbApi, color: "#00ADB5", label: "API Design" },
          { Icon: FaProjectDiagram, color: "#A78BFA", label: "Architecture" },
...
/* ─────────────────── SKILL ICON CARD ─────────────────── */
const SkillIcon = ({ Icon, color, label, isActive, onMouseEnter, onMouseLeave }) => {
  const [hovered, setHovered] = useState(false);
  const activeOrHovered = hovered || isActive;
  ...
  return (
    <motion.div
      ...
      animate={{
        y: activeOrHovered ? -4 : 0,
        scale: activeOrHovered ? 1.15 : 1,
      }}
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          border: `1px solid ${activeOrHovered ? color + "66" : "rgba(255,255,255,0.08)"}`,
          transition: "all 0.25s ease",
        }}
      >
        <Icon style={{ color: activeOrHovered ? color : "rgba(255,255,255,0.7)" }} />
      </div>
      <motion.span
        className="text-[9px] font-medium tracking-wide text-white/40 text-center leading-none"
        animate={{ opacity: activeOrHovered ? 1 : 0, y: activeOrHovered ? 0 : 4 }}
...
const AboutSection = () => {
  ...
  const [isHoveredAny, setIsHoveredAny] = useState(false);

  // Auto-glow cycle for skill icons
  useEffect(() => {
    if (aboutData[index].title !== "skills" || isHoveredAny) return;

    const totalIcons = aboutData[0].info.reduce(
      (acc, group) => acc + group.icons.length,
      0
    );

    const interval = setInterval(() => {
      setActiveGlowIndex((prev) => (prev + 1) % totalIcons);
    }, 2000);
  ...
  return (
    <SectionWrapper id="about" className="min-h-screen flex items-center justify-center bg-primary/30 py-24 lg:py-32 overflow-hidden">
      <Circles />
      ...
      <div className="container mx-auto px-4 flex flex-col xl:flex-row gap-12 relative z-10 xl:items-center">
                  ...
                  {aboutData[index].title === "skills" ? (
                    <div className="flex flex-col gap-6">
                      {(() => {
                        ...
                        return aboutData[index].info.map((group, gi) => (
                          <div key={gi}>
                              ...
                              {group.icons.map(({ Icon, color, label }, j) => {
                                ...
                                return (
                                  <SkillIcon
                                    ...
                                    color={color}
                                    label={label}
                                    isActive={activeGlowIndex === currentFlatIndex}
                                    onMouseEnter={() => {
                                      setIsHoveredAny(true);
                                      setActiveGlowIndex(currentFlatIndex);
                                    }}
                                    onMouseLeave={() => {
                                      setIsHoveredAny(false);
                                    }}
                                  />
                                );
                              })}
